...
for (const [fname, aspect] of Object.entries(FORMATE)) {
  ...
  const alleDuerren=[[],[],[]]; const alleDuerrenObj=[[],[],[]];
  ...
  for (let r = 0; r < LAEUFE; r++) {
    ...
    try {
      ...
      Rock.prototype.spawn = function (type, look, x, ...rest) {
        const i = idxVon(x);
        alleDuerren[i].push(jetzt-letzterStein[i]);
  ...
  const q=(a,p)=>{const b=[...a].sort((x,y)=>x-y);return b[Math.min(b.length-1,Math.floor(p*b.length))]??0;};
  console.log(`Duerre-Verteilung Steine (s)  p50 / p90 / p99 / max`);
  for(let i=0;i<3;i++) console.log(`  Bahn ${["links","MITTE","rechts"][i]}: ${q(alleDuerren[i],.5).toFixed(2)} / ${q(alleDuerren[i],.9).toFixed(2)} / ${q(alleDuerren[i],.99).toFixed(2)} / ${Math.max(...alleDuerren[i]).toFixed(2)}   (Anteil ueber 2.5 s: ${(100*alleDuerren[i].filter(v=>v>2.5).length/alleDuerren[i].length).toFixed(1)} %, ueber 5 s: ${(100*alleDuerren[i].filter(v=>v>5).length/alleDuerren[i].length).toFixed(1)} %)`);
